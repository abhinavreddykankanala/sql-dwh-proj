...
BEGIN
	...
	BEGIN TRY 
		...
		CASE WHEN UPPER(TRIM(cst_marital_status)) = 'M' THEN 'Married'   --Maps coded values to meaningful, user-friendly descriptions
		...
		--calculate end date as one day before the next start date, dateadd because it moves the date forward or backward whereas datediff gives the 
		...
		INSERT INTO silver.erp_px_cat_g1v2 (id, Category, Subcategory, Maintenance)
		SELECT
		id,
		Category,
		Subcategory,
